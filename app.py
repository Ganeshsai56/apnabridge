...
# --------- ADDED: CONTACT EMAIL SENDER (does NOT change any existing code) ---------
def send_contact_email(name, sender_email, phone, message_text):
    """
    Send contact form details to the ApnaBridge admin email (SENDER_EMAIL).
    """
    subject = "New contact enquiry from ApnaBridge website"
    body = (
        f"New contact message received from ApnaBridge website:\n\n"
        f"Name   : {name}\n"
    ...
    msg = MIMEText(body)
    msg["Subject"] = subject
    msg["From"] = SENDER_EMAIL      # from your SMTP account
    msg["To"] = SENDER_EMAIL        # send to yourself (admin inbox)
    reply_to = sender_email or SENDER_EMAIL
    ...
    try:
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as smtp:
            ...
            smtp.login(SENDER_EMAIL, SENDER_PASSWORD)
            smtp.send_message(msg)
        print(f"✅ Contact email sent from {sender_email} to admin inbox")
        return True
    except Exception as e:
        print("🔥 CONTACT email sending error:", e)
        return False
...
# --------- ADDED: CONTACT ROUTE (uses send_contact_email helper) ---------
@app.route("/contact", methods=["POST"])
def contact():
    """
    Receives contact form data from the website and emails it
    to the ApnaBridge admin inbox (SENDER_EMAIL).
    """
    try:
        data = request.get_json() or {}
        ...
        sent = send_contact_email(name, email, phone, message_text)
        if not sent:
            return jsonify({"message": "❌ Failed to send your message. Please try again later."}), 500

        return jsonify({"message": "✅ Your message has been sent successfully. We will contact you soon."}), 200

    except Exception as e:
        print("🔥 CONTACT ROUTE ERROR:", e)
        return jsonify({"message": "Server error"}), 500
